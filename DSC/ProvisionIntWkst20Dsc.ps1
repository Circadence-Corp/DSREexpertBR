Configuration SetupIntWkst20
{
    param(
        ...
        # AATP: Used to expose KrishG cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$KrishGCred,        
    ...
    #region AATP stuff
    [PSCredential]$KrishGDomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($KrishGCred.UserName)", $KrishGCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\KrishG")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst20.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
