Configuration SetupIntWkst09
{
    param(
        ...
        # AATP: Used to expose LillianaD cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$LillianaDCred,        
    ...
    #region AATP stuff
    [PSCredential]$LillianaDDomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($LillianaDCred.UserName)", $LillianaDCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\LillianaD")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst09.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
