Configuration SetupIntWkst14
{
    param(
        ...
        # AATP: Used to expose MacaulyW cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$MacaulyWCred,        
    ...
    #region AATP stuff
    [PSCredential]$MacaulyWDomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($MacaulyWCred.UserName)", $MacaulyWCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\MacaulyW")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst14.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
