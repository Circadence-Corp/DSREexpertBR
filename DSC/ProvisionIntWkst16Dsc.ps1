Configuration SetupIntWkst16
{
    param(
        ...
        # AATP: Used to expose TonyaM cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$TonyaMCred,        
    ...
    #region AATP stuff
    [PSCredential]$TonyaMDomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($TonyaMCred.UserName)", $TonyaMCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\TonyaM")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst16.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
