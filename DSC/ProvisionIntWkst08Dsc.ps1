Configuration SetupIntWkst08
{
    param(
        ...
        # AATP: Used to expose RaifeD cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$RaifeDCred,        
    ...
    #region AATP stuff
    [PSCredential]$RaifeDDomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($RaifeDCred.UserName)", $RaifeDCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\RaifeD")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst08.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
