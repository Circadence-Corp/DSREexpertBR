Configuration SetupIntWkst18
{
    param(
        ...
        # AATP: Used to expose JaxonR cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$JaxonRCred,        
    ...
    #region AATP stuff
    [PSCredential]$JaxonRDomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($JaxonRCred.UserName)", $JaxonRCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\JaxonR")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst18.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
