Configuration SetupIntWkst19
{
    param(
        ...
        # AATP: Used to expose OwainT cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$OwainTCred,        
    ...
    #region AATP stuff
    [PSCredential]$OwainTDomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($OwainTCred.UserName)", $OwainTCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\OwainT")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst19.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
