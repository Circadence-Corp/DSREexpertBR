Configuration SetupIntWkst11
{
    param(
        ...
        # AATP: Used to expose MorganP cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$MorganPCred,        
    ...
    #region AATP stuff
    [PSCredential]$MorganPDomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($MorganPCred.UserName)", $MorganPCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\MorganP")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst11.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
