Configuration SetupIntWkst07
{
    param(
        ...
        # AATP: Used to expose NolaM cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$NolaMCred,        
    ...
    #region AATP stuff
    [PSCredential]$NolaMDomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($NolaMCred.UserName)", $NolaMCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\NolaM")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst07.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
