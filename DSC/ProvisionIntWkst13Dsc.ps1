Configuration SetupIntWkst13
{
    param(
        ...
        # AATP: Used to expose ShannaM cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$ShannaMCred,        
    ...
    #region AATP stuff
    [PSCredential]$ShannaMDomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($ShannaMCred.UserName)", $ShannaMCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\ShannaM")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst13.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
