Configuration SetupIntWkst10
{
    param(
        ...
        # AATP: Used to expose KarinaM cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$KarinaMCred,        
    ...
    #region AATP stuff
    [PSCredential]$KarinaMDomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($KarinaMCred.UserName)", $KarinaMCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\KarinaM")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst10.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
