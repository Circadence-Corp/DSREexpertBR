Configuration SetupIntWkst03
{
    param(
        ...
        # AATP: Used to expose MuhammedO cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$MuhammedOCred,        
    ...
    #region AATP stuff
    [PSCredential]$MuhammedODomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($MuhammedOCred.UserName)", $MuhammedOCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\MuhammedO")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst03.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
