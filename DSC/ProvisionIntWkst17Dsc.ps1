Configuration SetupIntWkst17
{
    param(
        ...
        # AATP: Used to expose ZaineO cred to machine-DEPRECATED
        [Parameter(Mandatory=$true)]
        [ValidateNotNullOrEmpty()]
        [PSCredential]$ZaineOCred,        
    ...
    #region AATP stuff
    [PSCredential]$ZaineODomainCred = New-Object System.Management.Automation.PSCredential ("${NetBiosName}\$($ZaineOCred.UserName)", $ZaineOCred.Password)
    #endregion
    ...
    Node localhost
    {
        ...
        xGroup AddAdmins
        {
            ...
            MembersToInclude = @("$NetBiosName\Helpdesk", "$NetBiosName\ZaineO")
            Ensure = 'Present'
        ...
        xRemoteFile DownloadBginfo
		{
			...
			Uri = "https://github.com/Circadence-Corp/DSREexpertBR/raw/$Branch/Downloads/BgInfo/intwkst17.bgi"
            DependsOn = '[Computer]JoinDomain'
		}
